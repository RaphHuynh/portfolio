                ...
                <div className='flex items-center'>
                    ...
                    <div className='flex justify-center md:flex-row md:mr-0 md:ml-auto w-full md:w-auto'>
                        {props.textNav.map((text) => (
                            <a key={text.id} className="text-gray-100 md:mr-2 md:px-4 py-2 text-sm lg:text-base hover:bg-amber-400 hover:text-gray-900 rounded-full transition duration-300 ease-in-out sm:text-sm p-2 mx-2" href={text.id}><span >{text.content}</span></a>
                        ))}
                    </div>
                </div>
            </nav>
